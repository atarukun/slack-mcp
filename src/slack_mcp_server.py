...
# Note: The following user operations require special permissions
# and may not work with bot tokens

@mcp.tool("set_user_status")
async def set_user_status(status_text: str, status_emoji: str = "", expiration: int = 0) -> str:
    """
    Set the user's custom status. Requires user token (xoxp-) not bot token.
    
    Args:
        status_text: The status text to set
        status_emoji: The emoji to use (e.g., ':coffee:' or '☕')
        expiration: Unix timestamp when the status should expire (0 for no expiration)
    ...
    try:
        ...
        # Set status with rate limiting
        response = await make_slack_request(
            async_client.users_profile_set,
            profile=profile
        )
        
        if response:
            result = "✅ **Status Updated Successfully**\n\n"
            result += f"• **Status Text:** {status_text}\n"
            if status_emoji:
                result += f"• **Status Emoji:** {status_emoji}\n"
            if expiration > 0:
                exp_time = datetime.fromtimestamp(expiration).strftime('%Y-%m-%d %H:%M:%S')
                result += f"• **Expires At:** {exp_time}\n"
            else:
                result += "• **Expires:** Never\n"
            
            return result
        else:
            return "❌ Failed to set user status: API request failed (Note: This requires a user token, not a bot token)"
        ...
        if error_msg == 'not_allowed_token_type':
            return "❌ This operation requires a user token (xoxp-), not a bot token (xoxb-)"
...
async def update_user_profile(
    ...
    """
    Update user profile fields. Requires user token (xoxp-) not bot token.
    
    Args:
        display_name: Display name to set
        title: Job title to set
        phone: Phone number to set
        skype: Skype username to set
    ...
    try:
        ...
        if not profile:
            return "❌ No profile fields specified to update"
        ...
        if response:
            ...
            if skype is not None:
                result += f"• **Skype:** {skype}\n"
            
            return result
        else:
            return "❌ Failed to update profile: API request failed (Note: This requires a user token, not a bot token)"
...
@mcp.tool("set_user_presence")
async def set_user_presence(presence: str) -> str:
    """
    Set the user's presence (active or away). Requires user token (xoxp-) not bot token.
    ...
    try:
        ...
        if presence not in ['active', 'away']:
            return "❌ Invalid presence value. Must be either 'active' or 'away'"
        ...
        # Set presence with rate limiting
        response = await make_slack_request(
            async_client.users_setPresence,
            presence=presence
        )
        
        if response:
            emoji = "🟢" if presence == 'active' else "🟡"
            result = f"✅ **Presence Updated Successfully**\n\n"
            result += f"• **New Presence:** {emoji} {presence.title()}\n"
            
            return result
        else:
            return "❌ Failed to set user presence: API request failed (Note: This requires a user token, not a bot token)"
